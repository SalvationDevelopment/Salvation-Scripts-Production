...
--Script by dest
function c100211001.initial_effect(c)
	--synchro summon
	...
	e1:SetDescription(aux.Stringid(100211001,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_PZONE)
	e1:SetCountLimit(1,100211001)
	e1:SetCost(c100211001.spcost)
	e1:SetTarget(c100211001.sptg)
	e1:SetOperation(c100211001.spop)
	c:RegisterEffect(e1)
	--atk
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(100211001,1))
	...
	e2:SetCountLimit(1,100211101)
	e2:SetCondition(c100211001.condition)
	e2:SetTarget(c100211001.target)
	e2:SetOperation(c100211001.operation)
	c:RegisterEffect(e2)
	--pendulum
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(100211001,2))
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e3:SetCode(EVENT_DESTROYED)
	e3:SetProperty(EFFECT_FLAG_DELAY)
	e3:SetCondition(c100211001.pencon)
	e3:SetTarget(c100211001.pentg)
	e3:SetOperation(c100211001.penop)
	c:RegisterEffect(e3)
end
...
function c100211001.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c100211001.spop(e,tp,eg,ep,ev,re,r,rp)
...
function c100211001.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetCurrentPhase()~=PHASE_DAMAGE or not Duel.IsDamageCalculated()
end
function c100211001.filter(c)
...
function c100211001.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	Duel.SelectTarget(tp,c100211001.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
end
function c100211001.operation(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
...
function c100211001.pentg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckLocation(tp,LOCATION_SZONE,6) or Duel.CheckLocation(tp,LOCATION_SZONE,7) end
end
function c100211001.penop(e,tp,eg,ep,ev,re,r,rp)
