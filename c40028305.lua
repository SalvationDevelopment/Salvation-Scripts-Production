...
function c40028305.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(40028305,0))
...
function c40028305.filter(c)
	return c:IsFaceup() and c:IsSetCard(0x9a)
end
function c40028305.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c40028305.filter(chkc) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingTarget(c40028305.filter,tp,LOCATION_MZONE,0,1,e:GetHandler()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	Duel.SelectTarget(tp,c40028305.filter,tp,LOCATION_MZONE,0,1,1,e:GetHandler())
end
function c40028305.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	e5:SetDescription(aux.Stringid(40028305,1))
