--エネミーコントローラー
function c1149.initial_effect(c)
	--Activate
	...
	e1:SetCost(c1149.cost)
	e1:SetTarget(c1149.target)
	e1:SetOperation(c1149.activate)
	c:RegisterEffect(e1)
end
function c1149.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	e:SetLabel(1)
	return true
end
function c1149.filter(c,e)
	return c:IsFaceup() and c:IsCanBeEffectTarget(e)
end
function c1149.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1-tp) and chkc:IsFaceup() end
	if chk==0 then		
		e:SetLabel(0)
		return Duel.IsExistingTarget(Card.IsFaceup,tp,0,LOCATION_MZONE,1,nil)
	end
	local g=Duel.GetMatchingGroup(c1149.filter,tp,0,LOCATION_MZONE,nil,e)
	local cg=nil
	...
	if cg:GetCount()==0 then
		sel=Duel.SelectOption(tp,aux.Stringid(1149,0))
	elseif e:GetLabel()==1 and not Duel.CheckReleaseGroup(tp,nil,1,nil) then
		sel=Duel.SelectOption(tp,aux.Stringid(1149,0))
	else
		sel=Duel.SelectOption(tp,aux.Stringid(1149,0),aux.Stringid(1149,1))
...
function c1149.activate(e,tp,eg,ep,ev,re,r,rp)
