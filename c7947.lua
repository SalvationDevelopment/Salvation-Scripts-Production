...
function c7947.initial_effect(c)
	--fusion material
	c:EnableReviveLimit()
	aux.AddFusionProcCodeFun(c,13790826,c7947.mat_filter,1,false,false)
	--spsummon condition
...
function c7947.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) then
		...
		e3:SetTarget(c7947.reptg)
		e3:SetValue(c7947.repval)
...
function c7947.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return eg:IsExists(c7947.repfilter,1,nil,tp) end
	local g=eg:Filter(c7947.repfilter,nil,tp)
	local tc=g:GetFirst()
	while tc do
		tc:RegisterFlagEffect(7947,RESET_EVENT+0x1fc0000+RESET_PHASE+PHASE_END,EFFECT_FLAG_CLIENT_HINT,1,0,aux.Stringid(7947,2))
		tc=g:GetNext()
	end
