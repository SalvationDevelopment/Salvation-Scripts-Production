...
function c52145422.initial_effect(c)
	...
	e1:SetValue(c52145422.matcheck)
	c:RegisterEffect(e1)
	--Destroy
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(52145422,0))
	e2:SetCategory(CATEGORY_DESTROY)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e2:SetCode(EVENT_SPSUMMON_SUCCESS)
	e2:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e2:SetCountLimit(1,52145422)
	e2:SetCondition(c52145422.tdcon)
	e2:SetTarget(c52145422.tdtg)
	e2:SetOperation(c52145422.tdop)
...
function c52145422.matcheck(e,c)
	local ct=c:GetMaterial():Filter(Card.IsAttribute,nil,ATTRIBUTE_WATER):GetCount()
	e:SetLabel(ct)
end

function c52145422.tdcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetSummonType()==SUMMON_TYPE_SYNCHRO
end
function c52145422.tdtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c52145422.tdop(e,tp,eg,ep,ev,re,r,rp)
...
function c52145422.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c52145422.filter(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and
		Duel.IsExistingTarget(c52145422.filter,tp,LOCATION_GRAVE,0,1,e:GetHandler(),e,tp) end
