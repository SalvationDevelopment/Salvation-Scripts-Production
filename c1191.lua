...
function c1191.initial_effect(c)
	...
	e1:SetCondition(c1191.condition)
	e1:SetCost(c1191.cost)
	e1:SetTarget(c1191.target)
	e1:SetOperation(c1191.operation)
	c:RegisterEffect(e1)
end
function c1191.condition(e,tp,eg,ep,ev,re,r,rp)
...
function c1191.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local tc=e:GetLabelObject()
	if chkc then return chkc==tc end
	if chk==0 then return tc:IsCanBeEffectTarget(e) end
	Duel.SetTargetCard(tc)
end
function c1191.operation(e,tp,eg,ep,ev,re,r,rp,chk)
