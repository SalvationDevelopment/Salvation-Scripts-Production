...
function c6628343.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(6628343,2))
	e1:SetCategory(CATEGORY_TOHAND+CATEGORY_SEARCH)
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e1:SetCode(EVENT_REMOVE)
	e1:SetProperty(EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DELAY)
	e1:SetCountLimit(1,6628343)
	e1:SetCondition(c6628343.thcon)
	e1:SetTarget(c6628343.thtg)
	e1:SetOperation(c6628343.thop)
	c:RegisterEffect(e1)
	--Become material
	...
	e2:SetCondition(c6628343.condition)
	e2:SetOperation(c6628343.operation)
	c:RegisterEffect(e2)
end
...
function c6628343.condition(e,tp,eg,ep,ev,re,r,rp)
	return r==REASON_RITUAL
end
function c6628343.operation(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local rc=eg:GetFirst()
	while rc do
		if rc:GetFlagEffect(6628343)==0 then
			--Banish monster
			local e1=Effect.CreateEffect(c)
			...
			e1:SetCountLimit(1)
			e1:SetTarget(c6628343.rmtg1)
			e1:SetOperation(c6628343.rmop1)
			e1:SetReset(RESET_EVENT+0x1fe0000)
			rc:RegisterEffect(e1)
			--Chain attack
			local e3=Effect.CreateEffect(c)
			e3:SetDescription(aux.Stringid(6628343,1))
			e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
			e3:SetCode(EVENT_BATTLED)
			e3:SetCondition(c6628343.atcon)
			e3:SetOperation(c6628343.atop)
			e3:SetReset(RESET_EVENT+0x1fe0000)
			rc:RegisterEffect(e3)
			rc:RegisterFlagEffect(6628343,RESET_EVENT+0x1fe0000,0,1)
		end
		rc=eg:GetNext()
	end
end
function c6628343.rmtg1(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c6628343.rmop1(e,tp,eg,ep,ev,re,r,rp)
