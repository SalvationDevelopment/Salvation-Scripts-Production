...
function c14624296.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(14624296,0))
...
function c14624296.filter(c)
	return c:IsFaceup() and c:IsSetCard(0x9a)
end
function c14624296.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c14624296.filter(chkc) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingTarget(c14624296.filter,tp,LOCATION_MZONE,0,1,e:GetHandler()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	Duel.SelectTarget(tp,c14624296.filter,tp,LOCATION_MZONE,0,1,1,e:GetHandler())
end
function c14624296.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	e3:SetDescription(aux.Stringid(14624296,1))
