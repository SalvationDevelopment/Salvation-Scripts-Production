...
function c13790813.initial_effect(c)
	...
	e2:SetTarget(aux.TargetBoolFunction(c13790813.estarget))
	c:RegisterEffect(e2)
	--Atk Change
	...
	e3:SetCountLimit(1)
	e3:SetTarget(c13790813.atktg)
	e3:SetOperation(c13790813.atkop)
	c:RegisterEffect(e3)
	--search
...
function c13790813.estarget(c)
	return c:IsAttribute(ATTRIBUTE_LIGHT) and c:GetLevel()==1
end
function c13790813.tgfilter(c)
	return c:IsFaceup() and Duel.IsExistingMatchingCard(c13790813.cfilter,c:GetControler(),LOCATION_DECK+LOCATION_HAND,0,1,nil,c)
end
function c13790813.cfilter(c,tc)
	return c:IsType(TYPE_NORMAL) and c:IsType(TYPE_MONSTER) and c:IsAbleToGrave()
end
function c13790813.atktg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c13790813.atkop(e,tp,eg,ep,ev,re,r,rp)
