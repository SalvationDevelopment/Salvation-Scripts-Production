...
function c2648201.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsLocation(LOCATION_MZONE) and not c:IsFaceup() then return end
	local tc=Duel.GetFirstTarget()
	if Duel.GetLocationCount(tp,LOCATION_SZONE)<=0 or tc:GetControler()~=tp or not tc:IsFaceup() or not tc:IsRelateToEffect(e) or not c:CheckUniqueOnField(tp) then
