...
function c1100.initial_effect(c)
	...
	e2:SetOperation(c1100.desop)
	c:RegisterEffect(e2)
end
function c1100.filter(c)
	return c:IsFaceup() and c:IsControlerCanBeChanged()
end
function c1100.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1-tp) and c1100.filter(chkc) end
	if chk==0 then return true end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONTROL)
	local g=Duel.SelectTarget(tp,c1100.filter,tp,0,LOCATION_MZONE,1,1,nil)
...
function c1100.desop(e,tp,eg,ep,ev,re,r,rp)
